...
unordered_map<int, map<int, string> > alluserpipe;
...
void sig_usr(int signo)
{
    ...
    else if(signo == SIGINT)
    {
        ...
        shmctl(msgBufferid, IPC_RMID, NULL);
        shmctl(shmUserPipeTableid, IPC_RMID, NULL);
        shmctl(shmUPidTableid, IPC_RMID, NULL);
        exit(0);
    }
}
