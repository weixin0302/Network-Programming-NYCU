...
private:
    ...
    array<char, max_length> dst_msg;
    array<char, max_length> sor_msg;
    struct request{
      char VN;
      char CD;
...
public:
  session(ip::tcp::socket socket) : client_socket_(std::move(socket)), server_socket_(ioservice), resolver_(ioservice){}
  void start()
  {
    do_read();
  }
...
private:
  ...
  void do_read()
  {
    ...
    client_socket_.async_read_some(boost::asio::buffer(data_, max_length), [this, self](boost::system::error_code ec, std::size_t length)
    {
        if (!ec)
        {
  ...
  void do_connect_server()
  {
    ...
    if(pass_firewall())
    {
      ...
      server_socket_.async_connect(ep, [this, self](const boost::system::error_code &ec){
        if(!ec){
          send_reply(1, 1);
  ...
  void send_reply(int mode, int success)
  {
    ...
    client_socket_.async_send(boost::asio::buffer(buffer_, buffer_.size()), [this, self](boost::system::error_code ec, size_t){});
  }
  ...
  void do_communication(int server_to_client, int client_to_server)
  {
    ...
    if(server_to_client)
    {
      server_socket_.async_read_some(boost::asio::buffer(dst_msg, dst_msg.size()), [this, self](boost::system::error_code ec, size_t length){
        if(!ec)
        {
          // cout << "pass to client" <<endl;
          client_socket_.async_send(boost::asio::buffer(dst_msg, length), [this, self](boost::system::error_code ec, size_t){});
    ...
    if(client_to_server)
    {
      client_socket_.async_read_some(boost::asio::buffer(sor_msg, sor_msg.size()), [this, self](boost::system::error_code ec, size_t length){
        if(!ec)
        {
          // cout << "pass to server" <<endl;
          server_socket_.async_send(boost::asio::buffer(sor_msg, length), [this, self](boost::system::error_code ec, size_t){});
